...
export default function WizardFooter({
  ...
  const ContinueButton = ({ className }) => (
    <Button
      ...
      disabled={saving}
      className={cn(
        'gap-2 bg-linear-to-r from-[#3b1769] to-primary text-white shadow-sm hover:from-[#12101c] hover:to-primary/80',
        fixedContinue ? 'fixed bottom-6 right-6 z-50 shadow-lg' : className
      )}
    >
    ...
    <>
      ...
      <Card className="hidden border-border/60 shadow-sm sm:block">
        <CardContent className="flex flex-row items-center gap-3 p-4 sm:justify-between">
          ...
          {fixedContinue ? <div className="w-32" aria-hidden="true" /> : null}
          <ContinueButton />
        </CardContent>
      </Card>
