...
/**
 * GET /api/settings/day-plan-templates?search=
...
export async function GET(request) {
  try {
    ...
    const templates = await DayPlanTemplate.find(query).sort({ title: 1 }).lean()
    return Response.json({ templates })
  } catch (error) {
    console.error('List day plan templates error:', error)
...
export async function POST(request) {
  try {
    ...
    const template = await DayPlanTemplate.create({
    ...
    await recordAudit({
      ...
      action: 'create',
      summary: `Added day plan template "${template.title}"`,
      actor: authResult.user,
    })

    return Response.json({ template }, { status: 201 })
  } catch (error) {
    console.error('Create day plan template error:', error)
    return Response.json({ error: 'Internal server error' }, { status: 500 })
  }
}
